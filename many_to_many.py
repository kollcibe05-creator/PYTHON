...
from sqlalchemy.ext.declarative import Table, declarative_base
...
engine = create_engine("sql:///db.db") # The engine interprets the data from the db to the ORM and vise-versa
...
# Use of Association Object Model
class GameUser(Base):
...
################################################################
# Skipping the use of intermediaries entirely and using the Review to join the two entirely: Association Proxy (Hybrid Approach)
class Game(Base):
    __tablename__ = "games"
